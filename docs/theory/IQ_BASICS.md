...
Отсюда немедленно (аналогия: мы переводим «крутящуюся стрелку» в её координаты X и Y):
...
Замечание про `unwrap`: при вычислении мгновенной частоты важно снимать «скачки» фазы через `np.unwrap` — иначе частотный признак будет содержать искусственные импульсы.
...
Подсказка: перед вычислением `f_inst` нормируйте мощность, чтобы масштаб амплитуды не влиял на стабильность численного дифференцирования фазы.
